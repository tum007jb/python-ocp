...
if __name__ == "__main__":
    ...
    if result is not None:
        message_summary = ""
        message_summary += "<table border=\"1\">"
        for col1, col3 in result:
            if col3 == "True":
                message_summary += f"<tr><td> {col1} </td><td style=\"color:green;\">{col3}</td></tr>"
            else:
                message_summary += f"<tr><td> {col1} </td><td style=\"color:red;\">{col3}</td></tr>"
